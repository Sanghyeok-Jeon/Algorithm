...
def bfs(board, rx, ry, bx, by, move_history):
    q = deque([(rx, ry, bx, by, 1, move_history)])
    visited = set([(rx, ry, bx, by)])  # 방문한 상태 저장하는 집합

    while q:
        rx, ry, bx, by, cnt, move_history = q.popleft()
        ...
        for i in range(4):
            ...
            if board[nbx][nby] != 'O':  # 파란 구슬이 구멍에 빠지지 않은 경우
                ...
                if (nrx, nry, nbx, nby) not in visited:  # 빨간 구슬과 파란 구슬이 방문하지 않은 상태인 경우
                    q.append((nrx, nry, nbx, nby, cnt + 1, move_history + mode[i]))
                    visited.add((nrx, nry, nbx, nby))

    print(-1)    # 빨간 구슬이 구멍에 도달하지 못한 경우
    return
